...
impl ContactExtractor {
        ...
        for captures in self.email_regex.captures_iter(text) {
            if let Some(email_match) = captures.get(0) {
                ...
                if self.is_valid_contact_email(&email) && seen.insert(email.clone()) {
                    let context =
                        self.extract_context(text, email_match.start(), email_match.end());
        ...
        for captures in self.phone_regex.captures_iter(text) {
            if let Some(phone_match) = captures.get(0) {
                ...
                if phone.len() >= 10 && seen.insert(phone.clone()) {
                    let context =
                        self.extract_context(text, phone_match.start(), phone_match.end());
    ...
    fn calculate_email_confidence(&self, email: &str, context: &str, html: &str) -> f32 {
        ...
        if context_lower.contains("contact")
            || context_lower.contains("reach out")
            || context_lower.contains("email us")
            || context_lower.contains("get in touch")
        {
            confidence += 0.3;
        }
    ...
    fn calculate_phone_confidence(&self, phone: &str, context: &str) -> f32 {
        ...
        let context_lower = context.to_lowercase();
        if context_lower.contains("phone")
            || context_lower.contains("call")
            || context_lower.contains("tel")
            || context_lower.contains("contact")
        {
            confidence += 0.2;
        }
    ...
    pub fn is_contact_page(&self, html: &str, url: &str) -> bool {
        ...
        contact_indicators
            .iter()
            .any(|&indicator| html_lower.contains(indicator) || url_lower.contains(indicator))
    }
